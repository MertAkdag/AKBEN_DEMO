      ...
      <View style={styles.tickerBottom}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.tickerPrice, { color: colors.text }]}>
            {item.sell > 0
              ? `${item.unit}${item.sell.toLocaleString('tr-TR', {
...
export default function DashboardDetailsScreen() {
  ...
  const renderChart = () => {
    ...
    const path = pts.reduce((a, p, i) => {
      ...
      const pr = pts[i - 1];
      return `${a} C ${pr.x + (p.x - pr.x) * 0.4} ${pr.y}, ${pr.x + (p.x - pr.x) * 0.6} ${
        p.y
      }, ${p.x} ${p.y}`;
    }, '');
    const si = Math.min(selIdx, pts.length - 1);
      ...
      <GlassCard style={{ padding: CPAD }} delay={280} colors={colors} isDark={isDark}>
        <View style={styles.chartHead}>
          <View style={{ flex: 1 }}>
            ...
            <Animated.View
              style={[{ flexDirection: 'row', alignItems: 'baseline', gap: 4 }, valA]}
            >
              <Text style={[styles.chartBig, { color: colors.text }]}>
                {sp.v.toLocaleString('tr-TR', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </Text>
          ...
          <Animated.View
            style={[
              styles.dayPill,
              { backgroundColor: colors.primary + '14', borderColor: colors.primary + '20' },
              valA,
            ]}
          >
        ...
        <GestureDetector gesture={pan}>
          <View style={{ width: CHART_W, height: CHART_H }} collapsable={false}>
            <Svg width={CHART_W} height={CHART_H}>
              ...
              <Path
                d={`${path} L ${pts[pts.length - 1].x} ${CP.top + GH} L ${pts[0].x} ${
                  CP.top + GH
                } Z`}
                fill="url(#af)"
              />
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']}>
        <ScrollView contentContainerStyle={styles.scroll}>
          <ScreenHeader title="Detaylı Özet" subtitle="Yükleniyor..." />
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      >
        <ScreenHeader title="Detaylı Özet" subtitle={today} />
            ...
            <FlatList
              ...
              onScroll={(e: NativeSyntheticEvent<NativeScrollEvent>) => {
                const page = Math.round(
                  e.nativeEvent.contentOffset.x / (TICKER_CARD_W + TICKER_GAP),
                );
                if (page !== tickerPage) {
                  setTickerPage(page);
                  lightImpact();
